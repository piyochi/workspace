...
# コピー元とコピー先を指定
SOURCE_INIT="$HOME/workspace/.config/nvim/init.lua"
SOURCE_LUA="$HOME/workspace/.config/nvim/lua/"
DEST="$HOME/.config/nvim/"
DEST_LUA="$HOME/.config/nvim/lua/"
...
if [ $? -eq 0 ]; then
  echo "指定されたファイルとディレクトリをホームに正常にコピーしました。"
else
  echo "エラー: コピーに失敗しました。"
fi
